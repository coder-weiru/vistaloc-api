...
function processSecurityDefinitions(accountId, region, userPoolId) {
  ...
  securityDefinitions['vistaloc-custom-authorizer'] = {
    type: 'apiKey',
    name: 'Authorization',
    in: 'header',
  ...
  securityDefinitions['vistaloc-userPool-authorizer'] = {
    type: 'apiKey',
    name: 'Authorization',
    in: 'header',
...
function getApiDoc(restAPI, accountId, region, userPoolId) {
  let api = yaml.safeLoad(fs.readFileSync(path.join(__dirname, '..', 'swagger', 'vistaloc-api.yml')).toString());
  ...
  return cf.getStackOutputs().then((cfOutputs) => {
    ...
    return new Promise((resolve, reject) => {
      ag.getExport(params, function (err, data) {
        ...
        fs.writeFileSync(path.join(__dirname, '..', 'swagger', 'vistaloc-api-exported.yml'), yaml.safeDump(apiDoc));
...
function deleteApiExport() {
  return new Promise((resolve, reject) => {
    fs.unlink(path.join(__dirname, '..', 'swagger', 'vistaloc-api-exported.yml'), (err) => {
      if (err) {
        reject(err);
      }
